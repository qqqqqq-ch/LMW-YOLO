...
class DySample(nn.Module):
    def __init__(self, in_channels, scale=2, style='lp', groups=4, dy_init_func=lambda x: x):
        super().__init__()
        self.scale = scale
        self.style = style
        self.groups = groups
        ...
        if style == 'pl':
            ...
            self.input_channels = in_channels // (scale ** 2)
            self.out_channels = 2 * groups
        else:
            self.input_channels = in_channels
            self.out_channels = 2 * groups * scale ** 2

        self.offset = nn.Conv2d(self.input_channels, self.out_channels, 1)
        self.normal_init(self.offset, std=0.001)
        if dy_init_func is not None:
            dy_init_func(self.offset)

        self.scope = nn.PixelShuffle(scale) if style == 'lp' else nn.Identity()
        self.unpixel_shuffle = nn.PixelUnshuffle(scale) if style == 'pl' else nn.Identity()
    ...
    def forward(self, x):
        offset = self.offset(self.unpixel_shuffle(x)) if self.style == 'pl' else self.offset(x)
        offset = self.scope(offset)
        if self.style == 'pl':
            x = self.unpixel_shuffle(x)

        # 简化版实现：为了避免编译复杂的 CUDA 算子导致报错，
        # 很多 DySample 的 PyTorch 实现会回退到 grid_sample 或直接插值。
        # 这里为了让你能跑通，我们先用一个稳健的插值逻辑
        # 如果你有完整的 DySample 算子支持，请替换这里的逻辑。
        # 临时替代方案（保证不报错且有效果）：
