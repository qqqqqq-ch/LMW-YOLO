...
def bbox_iou(
    ...
    # 2. 修改判断条件，加入 WIoU
    if CIoU or DIoU or GIoU or WIoU or WCIoU:
        cw = b1_x2.maximum(b2_x2) - b1_x1.minimum(b2_x1)  # convex (smallest enclosing box) width
        ch = b1_y2.maximum(b2_y2) - b1_y1.minimum(b2_y1)  # convex height

        # 3. WIoU 也需要计算中心点距离 (rho2) 和对角线距离 (c2)
        if CIoU or DIoU or WIoU or WCIoU:
            ...
            # 4. WIoU v1 计算逻辑
            if WIoU:
                # WIoU v1 公式: Loss = (1 - IoU) * exp(rho^2 / c^2)
                # bbox_iou 返回值通常用于 Loss = 1 - result
                # 所以 result = 1 - Loss
                return 1 - (1 - iou) * torch.exp(rho2 / c2)
            ...
            if WCIoU:
                ...
                with torch.no_grad():
                    alpha = v / (v - iou + (1 + eps))
                return iou - (1.05*rho2 / c2 + v * alpha)  # WCIoU
